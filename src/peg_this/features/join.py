...
def join_videos(directory="."):
    ...
    try:
        ...
        if len(video_files) < 2:
            console.print(f"[bold yellow]Not enough video files in '{directory}' to join.[/bold yellow]")
            return
        ...
        try:
            first_video_path = os.path.abspath(os.path.join(directory, selected_videos[0]))
        ...
        for video_file in selected_videos:
            stream = ffmpeg.input(os.path.abspath(os.path.join(directory, video_file)))
            v = (
                stream.video
        ...
        output_stream = ffmpeg.output(joined[0], joined[1], output_path, **{'c:v': 'libx264', 'crf': 23, 'c:a': 'aac', 'b:a': '192k', 'y': None})

        if run_command(output_stream, "Joining and re-encoding videos...", show_progress=True):
            console.print(f"[bold green]Successfully joined videos into {output_path}[/bold green]")
        else:
